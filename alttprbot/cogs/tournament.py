...
from ..util import checks, speedgaming
        ...
        if c.Tournament[ctx.guild.id]['tournament'] == 'main':
            try:
                ...
                await dm_all_players_sg(
                    ctx,
                    episode_number,
                    (f"Settings submission not found at "
                      "<https://docs.google.com/spreadsheets/d/1GHBnuxdLgBcx4llvHepQjwd8Q1ASbQ_4J8ubblyG-0c/edit#gid=941774009>.  "
                     f"Please submit settings at <http://bit.ly/2Dbr9Kr> for episode `{episode_number}`!  "
                      "Once complete, re-run `$tourneyrace` command or contact your setup helper to have command re-ran."))
                raise
            except main.InvalidSettingsException:
                await dm_all_players_sg(
                    ctx,
                    episode_number,
                    (f"Settings submitted for episode `{episode_number}` are invalid!  "
                    "Please contact a tournament administrator for assistance."))
                raise
