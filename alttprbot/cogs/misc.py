...
class Misc(commands.Cog):
    ...
    async def pedestalgoal(self, ctx):
        await ctx.send(("> If it takes 2 hours its because GT is required, "
                        "which really isn't a thing anymore in pedestal goal games\n-Synack"))

    @commands.command(
        aliases=['crc32'],
        help=("Posts instructions on how to verify your ROM is correct, "
              "or how to get the permalink to your randomized game.")
    )
    async def rom(self, ctx):
        await ctx.send(("If you need help verifying your legally dumped Japan 1.0 rom file needed for ALTTPR,"
                        "check this out: http://alttp.mymm1.com/game/checkcrc/\n"
    ...
    async def holyimage(self, ctx, slug=None, game='z3r'):
        ...
        try:
            image = next(item for item in i if item["slug"] == slug or slug in item.get(
                "aliases", []) or str(item["idx"]) == slug)
        except StopIteration:
            raise Exception(
                'That holy image does not exist.  Check out <http://alttp.mymm1.com/holyimage/>')
